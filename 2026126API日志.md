...
| cupTypeList | List<ProductSpecPrice> | 是 | 杯型规格列表 | [{"specName": "大杯", "priceAdd": 2.00, "status": 1}] |
| tasteList | List<ProductSpecPrice> | 是 | 口味规格列表 | [{"specName": "正常糖", "priceAdd": 0.00, "status": 1}] |
| temperatureList | List<ProductSpecPrice> | 是 | 温度规格列表 | [{"specName": "常温", "priceAdd": 0.00, "status": 1}] |
| toppingList | List<ProductSpecPrice> | 否 | 小料规格列表 | [{"specName": "珍珠", "priceAdd": 2.00, "status": 1}] |
...
```json
{
  ...
  "description": "经典珍珠奶茶",
  "cupTypeList": [{"specName": "大杯", "priceAdd": 2.00, "status": 1}],
  "tasteList": [{"specName": "正常糖", "priceAdd": 0.00, "status": 1}],
  "temperatureList": [{"specName": "常温", "priceAdd": 0.00, "status": 1}],
  "toppingList": [{"specName": "珍珠", "priceAdd": 2.00, "status": 1}]
}
```
...
## ProductSpecPriceController

**基础路径**: `/api/productSpecPrice`
...
| 参数名 | 类型 | 必填 | 描述 | 示例 |
|--------|------|------|------|------|
| productSpecPrice | ProductSpecPrice | 是 | 商品规格对象 | 见下表 |

**ProductSpecPrice对象字段**:

| 字段名 | 类型 | 必填 | 描述 | 示例 |
|--------|------|------|------|------|
| productId | Long | 是 | 产品ID | 6 |
| specType | String | 是 | 规格类型（cup_type=杯型，taste=口味，temperature=温度，topping=小料） | cup_type |
| specName | String | 是 | 规格名称 | 大杯 |
| priceAdd | BigDecimal | 是 | 加价金额 | 2.00 |
| status | Integer | 是 | 状态（1=可用，0=不可用） | 1 |
...
### GET /generateSpecId/{productId}/{specType}
生成规格ID
